...
def main(cfg: DictConfig) -> None:
    ...
    with monitored_stage(cfg, f"train:{cfg.model.name}"):
        with ClearMLTracker(
            cfg,
            stage=f"train:{cfg.model.name}",
            task_type="training",
            extra_tags=[str(cfg.model.name)],
        ) as cml:
            ...
            metrics_json: dict[str, str | float] = {
                "model_name": str(cfg.model.name),
                "run_id": run_id,
            ...
            clearml_metrics: dict[str, float] = {
                "train_rmse": float(metrics_json["train_rmse"]),
                "train_mae": float(metrics_json["train_mae"]),
                "train_r2": float(metrics_json["train_r2"]),
                "val_rmse": float(metrics_json["val_rmse"]),
                "val_mae": float(metrics_json["val_mae"]),
                "val_r2": float(metrics_json["val_r2"]),
            }

            cml.log_metrics(clearml_metrics, series=str(cfg.model.name))
            cml.upload_artifact("train_metrics_json", metrics_path)
            cml.upload_artifact("trained_model_joblib", model_path)
            cml.register_model(
                model_path=model_path,
                model_name=f"{cfg.clearml.model_name}-{cfg.model.name}",
